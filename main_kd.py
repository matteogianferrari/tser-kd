...
from tser_kd.model import transfer_weights_resnet18_sresnet18, transfer_weights_resnet19_sresnet19, TSERKDLoss
from tser_kd.eval import run_eval
from tser_kd.training import run_kd_train
from tser_kd.dataset import StaticEncoder
from config_kd import args, args_dict
...
if __name__ == '__main__':
    ...
    # Loads the teacher state dict
    t_state_dict = torch.load(args.t_weight, map_location="cpu")

    # Creates the teacher model architecture
    ...
    for epoch_i in range(args.epochs):
        ...
        # Logging
        print(
            f"Time: {epoch_time:.1f}s | Train Loss: {train_total_loss:.4f} | Train Acc: {train_acc:.2f}% | "
            f"Val Loss: {val_loss:.4f} | Val Acc1: {val_acc1:.2f}% | Val Acc5: {val_acc5:.2f}% | LR: {curr_lr:.6f}"
        )

        run.log({
            "Train Total Loss": train_total_loss, "Train CE Loss": train_ce_loss, "Train KL Loss": train_kl_loss,
            "Train E Regularizer": train_e_reg, "Train Accuracy": train_acc,
